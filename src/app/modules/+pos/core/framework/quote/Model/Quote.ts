...
export class Quote extends DataObject {
  ...
  needSaveOnline(): boolean {
    const useGcOrRpDiscount = (this.getRewardPointData() && this.getRewardPointData()['use_reward_point']) || (this.getGiftCardData() && this.getGiftCardData()['base_giftcard_amount']);

    // If order has gift card item, we'll use online checkout to get gift card data by which we could print gift card barcode
    let hasGiftCardItem = false;
    if (this.getSyncedItems()) {
      hasGiftCardItem = !!_.find(this.getSyncedItems(), (i) => _.indexOf(RetailDataHelper.GIFT_CARD_TYPE_ID, i['type_id']) > -1);
    } else {
      hasGiftCardItem = !!_.find(this.getShippingAddress().getItems(), (i) => _.indexOf(RetailDataHelper.GIFT_CARD_TYPE_ID, i['type_id']) > -1);
    }

    return useGcOrRpDiscount || hasGiftCardItem;
  }
