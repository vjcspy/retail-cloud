...
@Pipe({name: 'priceOrderDetailFormat'})
export class PriceOrderDetailFormatPipe implements PipeTransform {
  ...
  transform(amount: any, priceFormat: any, ...args: any[]): number {
    if (isNaN(amount)) {
      return 0;
    }
    let format: any = this.globalPriceFormat;
    if (!!priceFormat) {
      format = _.extend(this.globalPriceFormat, priceFormat);
    } else {
