...
export class OrderDetailEffects {
  ...
  @Effect() noteOrder = this.actions$
    .ofType(
      OrderDetailActions.ACTION_NOTE_ORDER
    )
    .withLatestFrom(this.store$.select('general'))
    .switchMap((z: any) => {
      const noteData = z[0]['payload']['noteData'];

      return this.detailService.createNoteOrderRequest(noteData, z[1])
        .filter((data) => data.hasOwnProperty("items") && _.size(data['items']) === 1)
        .map((data) => {
          return data['items'][0];
        })
        .switchMap((order) => {
          ...
          return Observable.fromPromise(orderDB.save(orderDB, 'order_id'))
            .switchMap(() => {
              this.notify.success("update_note_order_success");
        ...
        .catch((e) => Observable.of(this.detailActions.noteOrderFailed("update_note_order_failed", e, false)));
    });
}
