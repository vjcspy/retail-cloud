TYRO = function() {
    ...
    var bootbox = window.bootbox || function(document, $) {
        var _locale = "en", _defaultLocale = "en", _animate = true, _backdrop = "static", _defaultHref = "javascript:;", _classes = "", _icons = {}, that = {};
        ...
        that.dialog = function(str, handlers, options) {
            var buttons = "", callbacks = [], options = options || {};
            if (handlers == null) {
                handlers = [];
    ...
    exports.IClientWithUI = function(apiKey, posProductData, documentToBindTo) {
        "use strict";
        var doc = documentToBindTo || document;
        var self = this;
        var channel;
        var expectedCallbacks = [ "transactionCompleteCallback" ];
        var iframeSourceUrl = "https://iclientsimulator.test.tyro.com/" + "iclient-with-ui.html?apiKey=" + apiKey + "&originOfPos=" + encodeURIComponent(SHARED_INTERFACE.getOriginOfPos());
        var bootstrapCssUrl = "https://iclientsimulator.test.tyro.com/" + "css/188ef5e6.bootstrap-v2.3.0-modal-excerpt.css";
        ...
        function createIFrameMarkup() {
            return '<iframe style="border: 0; overflow: hidden;" scrolling="no" id="iclient-headful-iframe" src="' + iframeSourceUrl + '" width=600 height=400 ></iframe>';
        }
        function createChannelToIframe() {
            var iframe = doc.getElementById("iclient-headful-iframe");
        ...
        function createIframeAndChannelInModalDialog() {
            if (channel) {
                throw new Error("Transaction already in progress.");
            }
            var iframeMarkup = createIFrameMarkup();
            var iclientElement = $('<div id="iclient-div-wrapper"><style id="dialog-css"> @import url(' + bootstrapCssUrl + ");</style></div>")[0];
            doc.body.appendChild(iclientElement);
            bootbox.dialog(iframeMarkup, [], {
                show: false,
                classes: "tyro-iclient-modal"
            });
            $("#dialog-css").load(function() {
                $(".tyro-iclient-modal").modal("show");
            });
            return createChannelToIframe();
        }
        function createHiddenIframeAndChannel() {
            if (channel) {
                throw new Error("Transaction already in progress.");
            }
            var iframeMarkup = createIFrameMarkup();
            var iframeElement = $(iframeMarkup).css("display", "none");
            doc.body.appendChild(iframeElement[0]);
            return createChannelToIframe();
        }
        function closeModalAndDestroyIframeAndChannel() {
            var $dialog = $(".tyro-iclient-modal");
            $dialog.on("hidden.bs.modal", function() {
                $("#iclient-div-wrapper").remove();
                $("#iclient-headful-iframe").remove();
            });
            $dialog.modal("hide");
            channel.destroy();
            channel = null;
        }
        function closeHiddenIframeAndDestroyChannel() {
            $("#iclient-headful-iframe").remove();
            channel.destroy();
            channel = null;
        }
        SHARED_INTERFACE.addSharedApiBindingsVersion1(self, createHiddenIframeAndChannel, closeHiddenIframeAndDestroyChannel);
        this.initiatePurchase = function(requestParams, transactionCallbacks) {
            ...
            SHARED_INTERFACE.validateTransactionAmounts("initiatePurchase", requestParams.amount, requestParams.cashout);
            SHARED_INTERFACE.validateIntegratedReceiptWidth("initiatePurchase", requestParams.integratedReceiptWidth);
            createIframeAndChannelInModalDialog();
            bindTransactionCompleteAndReceiptReceivedCallbacks(channel, transactionCallbacks);
            channel.notify({
                method: "purchase",
                params: $.extend(requestParams, {
        ...
        this.initiateRefund = function(requestParams, transactionCallbacks) {
            ...
            SHARED_INTERFACE.validateIntegratedReceiptWidth("initiateRefund", requestParams.integratedReceiptWidth);
            createIframeAndChannelInModalDialog();
            bindTransactionCompleteAndReceiptReceivedCallbacks(channel, transactionCallbacks);
            channel.notify({
                method: "refund",
        ...
        this.initiateIncrementPreAuth = function(requestParams, responseReceivedCallback) {
            ...
            createIframeAndChannelInModalDialog();
            SHARED_INTERFACE.bindReceiveLastMessageFromChannel(channel, closeModalAndDestroyIframeAndChannel, "incrementPreAuthResponse", responseReceivedCallback);
            channel.notify({
        ...
        this.continueLastTransaction = function(transactionCallbacks) {
            ...
            bindTransactionCompleteAndReceiptReceivedCallbacks(channel, transactionCallbacks);
            channel.notify({
                method: "continueLastTransaction"
            });
        };
        ...
        this.cancelCurrentTransaction = function(requestParams) {
            if (!channel) {
        ...
        this.pairTerminal = function(mid, tid, responseReceivedCallback) {
            ...
            createHiddenIframeAndChannel();
            SHARED_INTERFACE.bindReceiveMessageAndCleanUpIfDone(channel, closeHiddenIframeAndDestroyChannel, "pairingStatus", responseReceivedCallback);
            responseReceivedCallback({
                message: 'Please perform the "Authorise POS" function on your terminal',
                status: "inProgress"
            });
            channel.notify({
                method: "pairTerminal",
        ...
        function bindTransactionCompleteAndReceiptReceivedCallbacks(channel, transactionCallbacks) {
            SHARED_INTERFACE.bindReceiveLastMessageFromChannel(channel, closeModalAndDestroyIframeAndChannel, "transactionComplete", transactionCallbacks.transactionCompleteCallback);
            SHARED_INTERFACE.bindReceiveMessage(channel, "receiptReceived", transactionCallbacks.receiptCallback);
        }
    };
    return exports;
}();
//@ sourceMappingURL=iclient-with-ui-v1.source-map.json